...
Original file is located at
    https://colab.research.google.com/drive/1xxzrwUFdvFV01JSC5dS5X0o__VIFiYxx
"""
...
exp_seg = 6 ## Always 6 for single PGD
# seed_no_loc = 1
...
directory = "/cluster/scratch/pmayilvah/Week16_pgd/"+str(exp_eps)+"/"+str(seed_no_loc)+"/"
...
params = ["True", 32, "CIFAR", 125, 50000, 1, 250, eps_stack_temp[exp_eps-1], [eps_stack_temp[exp_eps-1]], segments_stack[exp_seg-1], 0.1, 'L2PGD', 20, True, True, False, False, seed_no]
...
if applyDilation and args['image_size']>=32:
    n = int(log2(args['image_size']/32))
    d1 = pow(2,(n+1)//2)
    d2 = pow(2,n//2)
else:
    d1 = 1
...
# resnet.fc = nn.Linear(512, args['nClasses']).to(device)
...
# segmenter_net_temp = fseg_NetCCFC(args['nClasses'], args['scale_capacity']).to(device)
...
# net.fcAvg = imgs2tensor(temp_images_untouched,args['number_of_segments'], device = device, nchans = 3)
...
# net_temp = NetCCFC(args['nClasses'], args['scale_capacity']).to(device)
# segmenter_net_temp = fseg_NetCCFC(args['nClasses'], args['scale_capacity']).to(device)
...
# segmenter_net_temp.fcAvg = imgs2tensor(temp_images_untouched,args['number_of_segments'], device = device, nchans = 3)
...
def compute_statistics(loader, net, criterion, PGD, fmodel, optimizer, sample_size, args = args):
  optimizer.zero_grad() # Making grads zero just in case (paranoid)
  eps_stack = args['epsilon_stack']
  device = args['device']
...
def compute_statistics_pgd_and_segmenter(loader, net, criterion, PGD, optimizer, sample_size, args = args):
  optimizer.zero_grad() # Making grads zero just in case (paranoid)
  eps_stack = args['epsilon_stack'] ## Change eps_stack to simply something with only eps
  device = args['device']
  nsegs = args['number_of_segments']
...
def compute_statistics_segmenter(loader, net, criterion, PGD, optimizer, sample_size, natural_stats = True, args = args):
  eps_stack = args['epsilon_stack']
  device = args['device']
...
def train_segmenter(epochs, args = args, path=""):
    
    global tr_loader, va_loader, te_loader, tr_loader_untouched, va_loader_untouched, te_loader_untouched
    # Assigning args to variables that we use
    
    device = args['device']
    pgd_steps = args['pgd_steps']
    segmentation_training = args['segmentation_training']
    PGD_training = args['PGD_training']
    eps_stack = args['epsilon_stack']
    attack = args['attack']
    scale_capacity = args['scale_capacity']
    ...
    if is_resnet:
        ...
        net = NetCCFC(nClasses, args['scale_capacity']).to(device)
    ...
    for e in range(epochs+1):
        ...
        ## Train set stats
        tr_vulnerabilities, tr_adv_loss, tr_adv_acc, tr_adv_norm_grads, tr_adv_images, tr_adv_segmented_images, tr_normal_images, tr_normal_segmented_images = compute_statistics_segmenter(tr_loader_untouched, net, criterion, PGD, optimizer, sample_size, False, args = args)
        for l in eps_stack:
        ...
        va_vulnerabilities, va_adv_loss, va_adv_acc, va_adv_norm_grads, va_adv_images, va_adv_segmented_images, va_normal_images, va_normal_segmented_images = compute_statistics_segmenter(va_loader_untouched, net, criterion, PGD, optimizer, sample_size, True, args = args)
        ...
        te_vulnerabilities, te_adv_loss, te_adv_acc, te_adv_norm_grads, te_adv_images, te_adv_segmented_images, te_normal_images, te_normal_segmented_images = compute_statistics_segmenter(te_loader_untouched, net, criterion, PGD, optimizer, sample_size, True, args = args)
        ...
        if e==0:
            ...
            if va_adv_loss[0.0] <= va_loss_best:
                ...
                args_save['best_epoch_loss'] = str(e)
                with open(directory+'params.txt', 'w') as file:
                    json.dump(args_save, file)
...
def train_pgd_and_segmenter(epochs, args = args, path=""):
    ...
    if is_resnet:
        ...
        ## Loading fseg_nets parameters from standard net
        init_net = NetCCFC(nClasses, args['scale_capacity']).to(device)
        net = fseg_NetCCFC(nClasses, args['scale_capacity']).to(device)
...
def train(epochs, args = args, path=""):
...
if args['segmentation_training'] and (not args['PGD_training']) and (not args['PGD_and_segmenter_training']):
    print("Starting segmenter training")
    train_segmenter(args['Epochs'])
elif args['PGD_training'] and (not args['segmentation_training'])and (not args['PGD_and_segmenter_training']):
    print("Starting PGD training")
    train(args['Epochs'])
elif args['PGD_and_segmenter_training'] and (not args['segmentation_training'])and (not args['PGD_training']):
    print("Starting PGD+segmenter training")
    train_pgd_and_segmenter(args['Epochs'])
else:
    pass
